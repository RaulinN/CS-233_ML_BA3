...
@test
def test_simulate_cover(scope):
    exercise_id = 'simulate_cover'
    simulate = helpers.resolve('simulate_cover', scope)

    test_data = helpers.get_test_data(exercise_id)
    te_trials = test_data["te_trials"]
    te_D = test_data["te_D"]
    te_list_of_N = test_data["te_list_of_N"]
    te_fractions = test_data["te_fractions"]

    gr_trials = test_data["gr_trials"]
    gr_D = test_data["gr_D"]
    gr_list_of_N = test_data["gr_list_of_N"]

    # Apply student's function and register results
    student_te_fractions = simulate(te_trials, te_D, te_list_of_N)
    student_gr_fractions = simulate(gr_trials, gr_D, gr_list_of_N)

    helpers.register_answer(exercise_id, student_gr_fractions, scope)

    helpers.compare_np_arrays(te_fractions, student_te_fractions, varname=exercise_id)

    return te_trials, te_D, te_list_of_N, student_te_fractions
